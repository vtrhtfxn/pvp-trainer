...
function duel(botDiff: DifficultyId, playerDiff: DifficultyId, seed: number, maxTicks = 20 * 60 * 4) {
  const m = new Match(kitById('sword'), DIFFICULTIES[botDiff], seed);
...
describe('bot duels', () => {
  ...
  it('never attacks on Practice, but still moves and heals', () => {
    ...
    for (let seed = 20; seed < 26; seed++) {
      ...
      if (Math.hypot(m.bot.pos.x, m.bot.pos.z + 6) > 1) moved++;
    }
    expect(moved).toBe(6);
  });
